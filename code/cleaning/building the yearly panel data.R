...
#WONDER data
setwd("C:/Users/Josh/OneDrive - University of Missouri/econometrics 2021/final project/data/raw/wonder query results/yearly")
...
#building the panel (year x state) (we want this to be balenced)
...
for(j in 1:50){
        ...
        for(i in 1:length(new_names)){
                shell[,2+i]<-ifelse(year(effective_dates[[i]])>=shell$year, 0, 1)}
        shell[is.na(shell)]<-0
...
legal_data<-legal_data %>% mutate(state=as.character(state), year=as.character(year))

panel<-merge(Wonder, legal_data, by=c("state", "year"))
...
#imputing suppressed values as 0
panel[is.na(panel)]<-9

#calculating opioid deaths as a fraction of total deaths
yearly_panel<-panel %>% mutate(Opioids_Death_Rate=Opioids_deaths/Number_of_Deaths_deaths)

#saving the panel
save(yearly_panel, file="C:/Users/Josh/OneDrive - University of Missouri/econometrics 2021/final project/data/clean/clean_yearly_panel.R", replace=T)
